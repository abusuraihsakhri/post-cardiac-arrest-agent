import pytest
from post_arrest_sentinel import TTMProtocolAuditorAgent, EEGPatternAnalyzerAgent, MultimodalNeuroScorerAgent, PostArrestCoordinator, main


def test_sub_agents():
    a1 = TTMProtocolAuditorAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = EEGPatternAnalyzerAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = MultimodalNeuroScorerAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = PostArrestCoordinator()
...
def test_domain_registry():
    from post_arrest_sentinel import DomainKnowledgeRegistry
